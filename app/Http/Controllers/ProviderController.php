...
class ProviderController extends Controller
{
    ...
    public function upcoming_trips222()
    {
	  $provider_id = Auth::user()->id;
	  $sql = "SELECT ur.*,u.first_name,u.last_name,u.email ,st.name as service_name  FROM user_requests ur JOIN users u ON ur.user_id = u.id
       JOIN service_types st on ur.service_type_id= st.id
	  where ur.provider_id = '$provider_id' AND ur.status != 'SCHEDULED'";
	 //echo $sql;die;
	  $query = DB::select($sql);
	  //echo "<pre>";print_r($query);die;
     //$fully = (new ProviderResources\TripController)->upcoming_trips();
     return view('provider.payment.upcoming222',['data'=>$query]);
    }
    /**
     * Show the application dashboard.
     *
